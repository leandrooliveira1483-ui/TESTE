...
!python colab_pipeline/03_train_predict_markets.py \
  ...
  --rolling-windows 6 \
  --rolling-window-size 120
```

- O `model_metrics.json` agora inclui também `rolling_backtest_report` e benchmark contra probabilidades implícitas de closing odds (quando há `--odds-file`).
...
## Teste rápido de invariantes probabilísticos

```python
!python tests/test_probability_invariants.py
```
