...
Antes de rodar no Colab:
    !pip install -q soccerdata pandas openpyxl pyarrow
"""
...
def fetch_understat(leagues: list[str], seasons: list[str]) -> pd.DataFrame:
    reader = sd.Understat(leagues=leagues, seasons=seasons)
    league_results = reader.read_league_results()
    league_results = _flatten_columns(league_results)
    league_results = league_results.reset_index(drop=False)
    league_results = _flatten_columns(league_results)
    return _normalize_understat_columns(league_results)
