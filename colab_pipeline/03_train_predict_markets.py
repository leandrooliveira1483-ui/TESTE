...
def fit_models(train_df: pd.DataFrame, num_cols: list[str], cat_cols: list[str], features: list[str]):
    ...
    m_home.fit(x_train, clean_train["home_goals"])
    m_away.fit(x_train, clean_train["away_goals"])
...
def apply_dixon_coles_adjustment(score_matrix: np.ndarray, home_lambda: np.ndarray, away_lambda: np.ndarray, rho: float) -> np.ndarray:
    ...
    for i in range(len(out)):
        lh = float(home_lambda[i])
        la = float(away_lambda[i])

        tau00 = max(1 - lh * la * rho, 1e-9)
        tau01 = max(1 + lh * rho, 1e-9)
        tau10 = max(1 + la * rho, 1e-9)
        tau11 = max(1 - rho, 1e-9)
